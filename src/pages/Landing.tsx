...
function Hero() {
  ...
  return (
    <section ref={ref} className="relative flex min-h-[100svh] flex-col overflow-hidden">
      {/* Flat onyx field — no gradients, no visual clutter */}
      <div className="bg-nova-aurora absolute inset-0" />
      <div className="grain absolute inset-0" />
      {/* WebGL layer — pushed to the right half so the copy stays clear */}
      <motion.div
        style={{ y: sceneY }}
        className="pointer-events-none absolute inset-y-0 left-0 right-0 z-0 lg:left-[38%]"
      >
      ...
      <motion.div style={{ opacity: fade }} className="relative z-10 mx-auto flex w-full max-w-6xl flex-1 flex-col px-6">
        {/* Text column — anchored to the left behind a solid, blurred panel */}
        <div className="flex flex-1 flex-col justify-center pb-10 pt-40 md:pb-16">
          <motion.div
            initial={{ opacity: 0, y: 44 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 1, delay: 0.2, ease: [0.22, 1, 0.36, 1] }}
            className="max-w-xl rounded-xl border border-white/10 bg-[#101016]/85 px-6 py-6 shadow-[0_32px_80px_-32px_rgba(0,0,0,0.9)] backdrop-blur-xl md:px-9 md:py-9"
          >
            <Eyebrow index="01" label="The Collection — FW26" tone="onDark" />
            <h1 className="font-display mt-8 text-[11vw] leading-[1.04] font-bold tracking-[-0.03em] text-foreground sm:text-7xl lg:text-[80px] lg:leading-[86.2px] lg:tracking-[-2.64px]">
              Designed for the
              <br />
              <span className="text-nova-gradient">next generation.</span>
            </h1>
            <p className="mt-7 max-w-md font-sans text-[16px] leading-[22px] tracking-[-0.18px] text-muted-foreground">
            ...
            <div className="mt-10 flex flex-wrap items-center gap-4">
              ...
              <NovaButton asChild variant="onDark" size="lg">
                <Link to="/shop">Explore NOVA</Link>
              </NovaButton>
            </div>
          </motion.div>
        </div>
        ...
        <Reveal>
          <div className="flex flex-wrap items-end justify-between gap-6">
            <div>
              ...
              <h2 className="font-display mt-7 max-w-xl text-4xl leading-[1.06] font-bold tracking-[-0.02em] sm:text-5xl">
                Objects of intent.
              </h2>
        ...
        <div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {CATEGORIES.map((cat, i) => (
            <Reveal key={cat.id} delay={i * 0.05}>
              ...
              >
                {/* Art panel — plain tone with an inner shadow, no gradients */}
                <div
                  className="absolute inset-0 bg-[#101016] transition-transform duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:scale-[1.05]"
                  style={{
                    boxShadow: `inset 0 -56px 72px -48px #000000, inset 0 0 0 1px ${CATEGORY_ART[cat.id]}26`,
                  }}
                />
                <span
                  aria-hidden
